...
| Multi-device session and key-rotation model | 🟡 | `pkg/e2ee` (succession, sessions, fan-out, strict bundle-set publication), `pkg/eventlog` (durable peer device ledger and local prekey publisher), `pkg/directory` + `pkg/daemon` (production refresh chain), `pkg/admission`, [`PREKEY_PUBLICATION.md`](PREKEY_PUBLICATION.md) | The model is durable and **enforced**: succession with classified rollback/equivocation and revocation defences, per-pair sessions and fan-out, crash-safe complete-set prekey generation/replenishment behind an isolated Endpoint signer, exact object publication, old-secret selection through signed expiry, pruning, and daemon-managed finalized delegation/policy → DHT → HTTPS peer refresh. Still 🟡 because the production local object sink + signed descriptor activation are not wired into the daemon, history synchronization and live message transport remain post-M0-R, and no live discovery/publication deployment evidence exists |
