...
| Messaging Contact Descriptor and DHT locator profile | 🟡 | `pkg/directory`, [`TOS_DHT_ADAPTER.md`](TOS_DHT_ADAPTER.md), [`HTTPS_OBJECT_SOURCE.md`](HTTPS_OBJECT_SOURCE.md) | The automatic route-neutral chain re-resolves finalized delegation, locator, descriptor, and prekeys. The production `tosutils-go` DHT adapter reads and publishes exact native TL locator values. The production HTTPS object source adds a strict digest-bound bundle-set wire profile, same-origin well-known publication path, finite budgets, no proxy/redirects, public-only DNS pinning, response bounds, positive vectors, and adversarial corpus; `NetworkRefreshSource` composes these with an injected finalized delegation source. Still missing: the out-of-band delegation implementation, daemon peer configuration/wiring, and live multi-node evidence |
...
| Multi-device session and key-rotation model | 🟡 | `pkg/e2ee` (succession, sessions, fan-out, strict bundle-set publication), `pkg/eventlog` (durable device ledger, `AdmitPublishedSet`), `pkg/directory` (refresh manager + native DHT/HTTPS sources), `pkg/admission` | The model is durable and **enforced**: succession with rollback/revocation defences, per-pair session derivation, per-event fan-out, and the admission gate refuses a revoked device with `CodeDeviceRevoked`. The route-neutral production retrieval chain now has native DHT locator and bounded HTTPS descriptor/prekey operations. Still 🟡 because finalized delegation bootstrap and daemon peer wiring are missing; live message transport remains post-M0-R |
