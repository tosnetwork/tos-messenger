...
| Multi-device session and key-rotation model | 🟡 | `pkg/e2ee` (succession, sessions, fan-out, strict bundle-set publication), `pkg/eventlog` (durable device ledger, `AdmitPublishedSet`), `pkg/directory` + `pkg/daemon` (production refresh chain), `pkg/admission` | The model is durable and **enforced**: succession with rollback/revocation defences, per-pair session derivation, per-event fan-out, revoked-device refusal, and daemon-managed finalized delegation/policy → DHT → HTTPS refresh into the device ledger. Still 🟡 because history synchronization and live message transport remain post-M0-R, and no live discovery deployment evidence exists |
