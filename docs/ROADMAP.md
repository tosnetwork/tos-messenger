...
| M0-R reachability and route decision | 🟡 | **Both collectors are prototypes, not complete route-decision collectors.** UDP direct-feasibility collector (`pkg/probe.Run`): 🟡 — it measures a datagram path and classifies mapping, but the classifier only produces `none`/`undetermined`/`endpoint-independent`/`address-and-port-dependent`, so it cannot yet generate the `address-dependent` and `symmetric` NAT scenarios the example policy requires, and mapping/filtering are not measured as separate dimensions. ADNL direct-establishment collector (`pkg/probe.RunADNL`): 🟡 — one session by construction, done-signalling via the coordinator, but it is IPv4-only, measures a single ping round trip (no keepalive, survival, reconnect, Wi-Fi↔mobile, or reliable-transfer), makes no proxy/tunnel attempt (so the report's `tunnel-first` branch is unreachable from collected evidence and a direct-failure study skews to `relay-required`), and rides `tonutils-go`'s ADNL rather than the TOS node's own stack. **Scenario strata (family, reachability, NAT, carrier, mobility) are self-declared by the endpoint and signed, not re-derived by the verifier from coordinator observations.** The report tool exits non-zero for any study that supports no route decision (`exitCode`, tested). **No study of either kind has been run.** Proxy/Tunnel measurement ⬜, TOS-native cross-check ⬜, verifier-derived scenario classification ⬜, full mapping/filtering NAT taxonomy ⬜, the study itself ⬜ |
...
|---|---:|---|---|
| M0-R measured reachability study and route-strategy decision | 🟡 | `pkg/reachability`, `pkg/probe` (UDP and ADNL), `cmd/tos-reachability*` | Collectors are prototypes (see the M0-R milestone row for the specifics): the NAT classifier does not yet produce the full mapping/filtering taxonomy the example policy requires; scenario strata are endpoint-declared and signed, not re-derived by the verifier from coordinator observations; the ADNL collector is IPv4-only, single-ping, proxy/tunnel-free, and rides `tonutils-go` rather than the TOS node's own ADNL. Beyond the tooling, the study itself — multiple operators, multiple sites, real networks — has not been run |
