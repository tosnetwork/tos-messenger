...
| Multi-device session and key-rotation model | 🟡 | `pkg/e2ee` (succession, sessions, fan-out, strict bundle-set publication), `pkg/eventlog` (device-local private prekeys, public-only Endpoint aggregation, durable peer ledger), `pkg/directory` (static HTTPS publisher, ordered Descriptor activation, signer-backed native DHT publication), `pkg/daemon` (peer refresh), `pkg/admission`, [`PREKEY_PUBLICATION.md`](PREKEY_PUBLICATION.md) | The model is durable and **enforced** without centralized device secrets: each device generates/retains its own material; the Endpoint aggregates only signed public contributions; complete-set replenishment, exact retry, old-secret expiry/pruning, immediate local revocation, classified rollback/equivocation, immutable prekey→Descriptor ordering, signed locator preparation, and native envelope signing through a verified `crypto.Signer` are tested. Still 🟡 because device-contribution collection and publication scheduling are not wired into the daemon, history synchronization and live message transport remain post-M0-R, and no live discovery/publication evidence exists |
