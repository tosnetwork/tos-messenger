...
| Messaging Contact Descriptor and DHT locator profile | 🟡 | `pkg/directory`, [`TOS_DHT_ADAPTER.md`](TOS_DHT_ADAPTER.md) | The automatic route-neutral chain re-resolves finalized delegation, locator, descriptor, and prekeys. The production `tosutils-go` DHT adapter now reads and publishes locators under the exact native TL key, re-verifies both native signature layers, the signature update rule, owner, TTL, bounds, and the inner Endpoint signature; tests cross-check the live TL encoder and substitution failures. Still missing: a bounded production descriptor/prekey object source, daemon peer configuration/wiring, and live multi-node DHT evidence |
...
| Multi-device session and key-rotation model | 🟡 | `pkg/e2ee` (succession, sessions, fan-out), `pkg/eventlog` (durable device ledger, `AdmitPublishedSet`), `pkg/directory` (refresh manager + native DHT adapter), `pkg/admission` | The model is durable and **enforced**: succession with rollback/revocation defences, per-pair session derivation, per-event fan-out, and the admission gate refuses a revoked device with `CodeDeviceRevoked`. The route-neutral refresh manager and production locator DHT operation exist. Still 🟡 because the bounded descriptor/prekey object source and daemon peer wiring are missing; live message transport remains post-M0-R |
