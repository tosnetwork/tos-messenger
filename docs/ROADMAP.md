...
| M3 OpenFox and conversation-to-commerce integration | 🟡 | Foundations only: typed commerce, mandates, budgets, durable negotiations and the verification contract exist. The daemon now verifies its own endpoint delegation through a concrete finalized Agent resolver, but the finalized quote resolver is not injected into an execution path. No transport, wallet, or execution integration |
...
| Messenger-specific encrypted attachment protocol | 🟡 | `pkg/attachments`, `artifact.encrypted`, [`ENCRYPTED_ATTACHMENTS.md`](ENCRYPTED_ATTACHMENTS.md) | Route-neutral AES-256-GCM chunk encryption, secret E2EE Reference, ordered ciphertext manifest/content addresses, strict outer-event binding, expiry, resume planning, local size/media policy, candidate vector and adversarial codec cases exist. Still missing: storage transport/authentication, locator SSRF policy, remote deletion/GC guarantees, sandbox/scanner integration and live interrupted-transfer evidence; commercial storage remains Expansion-Gate locked |
