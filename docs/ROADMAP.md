...
| Multi-device session and key-rotation model | 🟡 | `pkg/e2ee` (succession, sessions, fan-out, strict bundle-set publication), `pkg/eventlog` (device-local private prekeys, fixed-roster public contribution collection, public-only Endpoint aggregation, durable peer ledger), `pkg/prekeyapi` (separate strict device-facing public API), `pkg/directory` (static HTTPS publisher, ordered Descriptor activation, signer-backed native DHT publication), `pkg/daemon` (peer refresh), `pkg/admission`, [`PREKEY_PUBLICATION.md`](PREKEY_PUBLICATION.md), [`PREKEY_DEVICE_API.md`](PREKEY_DEVICE_API.md) | The model is durable and **enforced** without centralized device secrets: each device generates/retains its own material; the Endpoint durably stages only planned signed public contributions and finalizes only an exact complete roster; the separate bounded Unix API exposes no other device's bundle or authority and automatically finalizes the last contribution. Complete-set replenishment, exact retry, old-secret expiry/pruning, immediate local revocation, classified rollback/equivocation, crash-recoverable collection→publication ordering, immutable prekey→Descriptor ordering, signed locator preparation, and native envelope signing through a verified `crypto.Signer` are tested. Still 🟡 because generation configuration, the third listener, and publication scheduling are not wired into the daemon, history synchronization and live message transport remain post-M0-R, and no live discovery/publication evidence exists |
