...
| Network binding at finalisation | a negotiation is bound to an expected TOS network when it is created, and Finalize accepts a resolved quote only if the quote's network equals that binding -- the network id and both genesis hashes, because the same workchain, account and code hashes can exist on another network and a network id alone is not identity. The binding is persisted and restored so it survives a restart, and a mismatch settles the exchange to rejected. **This equality check is the bounded closure. The deeper fix -- folding the network domain into the canonical digests of the terms, mandate and asset, so a cross-network replay of identical terms hashes to a different commitment rather than relying on a separate check -- is a wire/digest change with broad ripple, deliberately left out of scope; it belongs with the network domain's canonical-preimage freeze noted below** | `negotiation.New`, `negotiation.Finalize`, `negotiation.Snapshot` |
...
| Durable budget in the local-API spend path | **deferred, design-dependent.** The review is right that the auto-consume path enforces only the per-spend ceiling, not a durable cross-action total, so a mandate's `MaxTotal` is not held across separate spends. Wiring a durable reservation there needs the budget scoped per mandate, but `eventlog.OpenBudgetLedger` keys a budget by *asset*, so two mandates sharing an asset would share one ledger — the per-asset-vs-per-mandate scoping, and the reserve/commit/release lifecycle the local-API path would own, are a design decision not yet made. The economic-execution one-shot above bounds a *single* purchase to one authorisation; bounding the *sum* of distinct purchases against `MaxTotal` durably is the remaining half | pending |
