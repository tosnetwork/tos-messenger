...
type CorpusEntry struct {
	...
	// Wire is the transport bytes to feed the target decoder: the JSON
	// document for JSON targets, lowercase hex for binary ones.
...
func TestAdversarialCorpus(t *testing.T) {
	...
	for _, entry := range entries {
		...
		if err := decode(raw); err == nil {
			t.Fatalf("%s (%s) was accepted, but the corpus says it must be refused: %s",
				entry.Name, entry.Target, entry.Reason)
		}
	}
...
func generateCorpus(t *testing.T) []CorpusEntry {
	t.Helper()
	var corpus []CorpusEntry
	add := func(name, target, wire, reason string) {
		corpus = append(corpus, CorpusEntry{Name: name, Target: target, Wire: wire, Reason: reason})
	}
	addBinary := func(name, target, hexWire, reason string) {
		corpus = append(corpus, CorpusEntry{Name: name, Target: target, Wire: hexWire, Binary: true, Reason: reason})
	}
