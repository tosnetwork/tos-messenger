...
func run(configPath string, checkOnly bool) error {
	...
	if checkOnly {
		fmt.Printf("configuration is valid: state_dir=%s socket=%s transport=%s\n",
			config.StateDir, config.SocketPath, config.Transport)
		return nil
	}
	...
	fmt.Printf("listening=%s state_dir=%s transport=%s\n",
		instance.SocketPath(), config.StateDir, config.Transport)
