...
func DecisionBytes(request Request, challenge string) ([]byte, error) {
	...
	if request.Mandate != nil {
		...
		canon.Text(buffer, request.Mandate.Asset)
		buffer.WriteByte(request.Mandate.Decimals)
		canon.Uint64(buffer, request.Mandate.MaxTotalUnits)
		canon.Uint64(buffer, request.Mandate.ApprovalAbove)
